...
<div class="flex flex-col items-center justify-center h-full gap-3 sm:gap-4 md:gap-6 p-4 landscape-compact landscape-scroll">
  ...
  <div class="text-center">
    <div class="text-sm sm:text-lg text-gray-400 uppercase tracking-widest mb-1 sm:mb-2">Victory</div>
    <h2 class="text-3xl sm:text-4xl md:text-5xl font-black" style="color: {winnerColor}; text-shadow: 0 0 30px {winnerColor}66;">
      {winnerName} WINS!
    </h2>
  </div>

  <!-- Match stats -->
  <div class="bg-gray-900/90 rounded-xl p-4 sm:p-6 w-full max-w-xs sm:max-w-sm md:max-w-[420px] space-y-3 sm:space-y-4 border border-gray-800">
    <!-- P1 stats -->
    <div class="flex items-center justify-between py-2 sm:py-3 border-b border-gray-800">
      <div class="flex items-center gap-2 sm:gap-3">
        <span class="text-xs font-bold text-blue-400 bg-blue-400/20 px-2 py-0.5 rounded">P1</span>
        <div>
          <div class="font-bold text-sm sm:text-base" style="color: {colorHex(gameStore.p1Character)}">{p1FighterName}</div>
          <div class="text-xs text-gray-500">{gameStore.p1Name}</div>
        </div>
      </div>
      <div class="text-right">
        <div class="text-xl sm:text-2xl font-black font-mono">{gameStore.p1Damage}<span class="text-xs sm:text-sm text-gray-500">%</span></div>
        <div class="flex gap-1 justify-end mt-1">
          {#each Array(gameStore.totalStocks) as _, i}
            <div class="w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full"
    ...
    <div class="flex items-center justify-between py-2 sm:py-3">
      <div class="flex items-center gap-2 sm:gap-3">
        ...
        <div>
          <div class="font-bold text-sm sm:text-base" style="color: {colorHex(gameStore.p2Character)}">{p2FighterName}</div>
          <div class="text-xs text-gray-500">{gameStore.p2Name}</div>
        </div>
      </div>
      <div class="text-right">
        <div class="text-xl sm:text-2xl font-black font-mono">{gameStore.p2Damage}<span class="text-xs sm:text-sm text-gray-500">%</span></div>
        <div class="flex gap-1 justify-end mt-1">
          {#each Array(gameStore.totalStocks) as _, i}
            <div class="w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full"
  ...
  <!-- Actions -->
  <div class="flex flex-col sm:flex-row gap-2 sm:gap-3 w-full max-w-xs sm:max-w-sm md:max-w-64">
    <button
      class="bg-red-600 hover:bg-red-500 text-white font-bold py-2.5 sm:py-3 px-6 rounded-lg transition-colors text-base sm:text-lg flex-1"
      onclick={rematch}
    >
      REMATCH
    </button>
    <button
      class="bg-gray-700 hover:bg-gray-600 text-white py-2.5 sm:py-3 px-6 rounded-lg transition-colors flex-1"
      onclick={characterSelect}
    >
      Character Select
    </button>
    <button
      class="bg-gray-800 hover:bg-gray-700 text-gray-300 py-2 px-6 rounded-lg transition-colors flex-1"
      onclick={mainMenu}
    >
      Main Menu
    </button>
