...
<!-- Top: Timer only -->
<div class="absolute top-0 left-0 right-0 pointer-events-none flex justify-center pt-1 sm:pt-2">
  <div class="bg-black/60 rounded-lg px-3 sm:px-4 py-1 sm:py-1.5 border border-gray-700/50">
    <span class="text-lg sm:text-2xl font-bold font-mono leading-none"
...
<div class="absolute bottom-0 left-0 right-0 pointer-events-none">
  <div class="flex justify-center gap-4 sm:gap-8 pb-2 sm:pb-3 px-2 sm:px-4">

    <!-- P1 Panel -->
    <div class="relative flex flex-col items-center">
      <!-- Character name -->
      <div class="text-[10px] sm:text-xs font-bold tracking-wider uppercase mb-0.5 sm:mb-1"
           style="color: {colorHex(gameStore.p1Character)}">
        {p1FighterName}
      </div>

      <!-- Damage panel -->
      <div class="relative bg-gradient-to-b from-gray-900/90 to-black/95 rounded-lg sm:rounded-xl px-4 sm:px-8 py-2 sm:py-3 border-2 min-w-[100px] sm:min-w-[160px] text-center"
           style="border-color: {colorHex(gameStore.p1Character)}40;">

        <!-- Player label -->
        <div class="absolute -top-2 sm:-top-2.5 left-2 sm:left-3 bg-blue-500 text-white text-[8px] sm:text-[10px] font-bold px-1.5 sm:px-2 py-0.5 rounded-full">
          P1
        </div>

        <!-- Damage percentage -->
        <div class="text-2xl sm:text-4xl font-black font-mono leading-none tracking-tight transition-all duration-75"
             style="color: {damageColor(gameStore.p1Damage)}; text-shadow: {damageGlow(gameStore.p1Damage)};">
          {gameStore.p1Damage}<span class="text-lg sm:text-2xl">%</span>
        </div>

        <!-- Stock icons -->
        <div class="flex justify-center gap-1 sm:gap-1.5 mt-1 sm:mt-2">
          {#each Array(gameStore.totalStocks) as _, i}
            <div class="w-2.5 h-2.5 sm:w-3.5 sm:h-3.5 rounded-full transition-all duration-200"
    ...
    <div class="flex items-center">
      <span class="text-gray-600 text-sm sm:text-lg font-bold">VS</span>
    </div>
    ...
    <div class="relative flex flex-col items-center">
      ...
      <div class="relative bg-gradient-to-b from-gray-900/90 to-black/95 rounded-lg sm:rounded-xl px-4 sm:px-8 py-2 sm:py-3 border-2 min-w-[100px] sm:min-w-[160px] text-center"
        ...
        <!-- Player label -->
        <div class="absolute -top-2 sm:-top-2.5 right-2 sm:right-3 text-white text-[8px] sm:text-[10px] font-bold px-1.5 sm:px-2 py-0.5 rounded-full"
        ...
        <div class="text-2xl sm:text-4xl font-black font-mono leading-none tracking-tight transition-all duration-75"
          ...
          {gameStore.p2Damage}<span class="text-lg sm:text-2xl">%</span>
        </div>
